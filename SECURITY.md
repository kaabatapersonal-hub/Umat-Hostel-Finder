...
| 8 | Low (but user-visible) | `scripts/security-audit.mjs`'s bootstrap owner-hostel fixture was reused indefinitely across runs and never cleaned up — harmless against an empty dev database, but once real hostels went live, `"[Security Audit] Renamed via pending edit"` at `"Security Audit Zone"` sat in the real, public, anon-readable home feed between audit runs, visible to real users. Found by a pre-launch pass, not by the audit script itself (nothing in the matrix checks that the audit doesn't pollute production). | **Fixed** — the script now deletes this hostel at the end of every run (`reviews`/`saved_hostels` FK-cascade, so no orphaned rows); the very next run creates a fresh one. The one live instance was deleted from production directly. If the audit is ever run against a database with real users again, re-check the public feed afterward regardless — this class of bug (a test fixture treated as safe to leave live) could recur elsewhere. |
