# Applying the Session 4.5 migration

One migration this session:
`supabase/migrations/20260702221215_room_types_pricing_facilities_contact.sql`.
It **alters existing data** (drops `price`, `room_type`, `room_images`,
backfills `room_types`) — safe here because it's still all test data, but
...
## 2. Seed richer test data

Run `supabase/seed_session4_5.sql` in the same editor. It gives Green Acres
two room types with photos (tests the price range + multi-row breakdown)
and a `call_number`, gives Sunset Lodge three room types with no photos yet
(tests price-only rows), and leaves Kings Court with its single backfilled
room type and no `call_number` (tests the single-price card + hidden Call
button).

## 3. Verify

- `npm run dev`, open `/` — Green Acres and Sunset Lodge show a price
  *range* pill; Kings Court and everything else show a single price, no
  stray dash.
- Toggle "Under GHS 2,000" — should still match hostels whose **cheapest**
  room is under 2,000 (e.g. Sunset Lodge, min 1,300, now qualifies even
  though its most expensive room is 3,500).
- Open Green Acres' details page — two room-type rows (1 in a room, 2 in a
  room), each with its own price and photos, sorted by occupancy.
- Open Sunset Lodge's details page — three rows, no photos on any (tests
  price-only rows rendering cleanly).
- Green Acres shows both a WhatsApp button and a Call button; Kings Court
  shows WhatsApp only (no Call button — no call_number).
- Tap either contact button — reveals **both** numbers at once (or just the
  WhatsApp one if there's no call_number).
- A hostel with an unrecognized facility string (once you add one) should
  render with the generic fallback icon, not break.
